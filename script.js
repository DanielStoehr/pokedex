...
async function setPokemonList() {
    ...
    pokemonList = await Promise.all(
        list.map(async (item) => await loadPokemon(item.url))
    );
}
...
function openDetail(id) {
    ...
    document.getElementById(
        "detail-pokemon-id"
    ).innerHTML = `#${formatIdToString(pokemon.id)}`;
    document.getElementById("detail-types").innerHTML = renderTypes(
        pokemon.types
    );
...
function renderDetailBase() {
    ...
    for (item of values) {
        const progressClass =
            item.value < 50 ? "progress-red" : "progress-green";
        content += `
            <tr>
                <th>${item.name}</th>
                <td>${item.value}</td>
